...
void DFS(struct Graph *graph, Vertex *vertex, int **visited, char **results,
         int initial_id);

void add_edge(Graph **graph, char *start_name, char *direction_name,
              int weight);
int modify_weight_in_edge(Graph *graph, char *start_name, char *direction_name,
                          int weight, int new_weight);
Edge *search_for_edge(Graph *graph, char *start_name, char *direction_name,
                      int weight, int *error);
void print_edges(Graph *graph, char *name);
int delete_edge(Graph *graph, char *start_name, char *direction_name,
                int weight);
int delete_edges(Graph *graph, char *start_name, char *direction_name);
void delete_edges_without_search(Graph *graph, Vertex **vertex,
                                 char *direction_name);
